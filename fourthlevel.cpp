...
QSqlQuery fourthlevel::getAllInfo(QString str){
    QSqlQueryModel model;// = new QSqlQueryModel();
    QSqlQuery *query = new QSqlQuery();
    if(str == "Объекты")
        query->prepare("SELECT * FROM GetObject()");
    if(str=="Персонал")
        query->prepare("SELECT * FROM PersonalInfo()");
    if(str=="Добыча")
...
QSqlQuery fourthlevel::getcityInfo(QString str, QString city){
    ...
    if(str=="Персонал"){
        query->prepare("SELECT * FROM cityPersonalInfo(?)");
        query->bindValue(0,city);
    }
    if(str=="Добыча"){
